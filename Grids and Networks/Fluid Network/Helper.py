...
class Notification():
    ...
    def user_junction_detail(self, i):
        junc_name = input(f"What is the name of junction number {i}: ")
    ...
    def case_file_import_confirm(self):
...
class CaseFileReader():
    ...
    def Pickle_reader(self):
        ...
        net = ppp.from_pickl(os.path.join(input_directory, 'Case Files/Pickle files/' + Pickle))
    ...
    def JSON_reader(self):
        for file_name in os.listdir("G:\Project - OOP\Fluid Network\Case Files\Json files"):
            if file_name.endswith(".json"):
                Json = file_name
        input_directory = os.path.dirname(os.path.realpath('__file__'))
        net = ppp.from_json(os.path.join(input_directory, 'Case Files/Json files/' + Json))
        warnings.filterwarnings('ignore')
        return net
    
    def Excel_reader(self):
       for file_name in os.listdir("G:\Final-Project\Grids and Networks\Fluid Network\Case Files\Excel files"):
...
class CaseFileEditor():
    ...
    def network_elements_analyzer_editor(self, elem_name):
...
class Excel_pipeline_creator():
    ...
    def Excel_file_analyzer(self):
        ...
        for i in range(len(self.sheets)):
            if self.sheets[i] == "junction":
                excel = pd.read_excel(self.Excel_file, sheet_name = self.sheets[i], index_col = 0)
                for junc in excel.index:
                    globals()["junction" + str(junc)] = ppp.create_junction(self.net, pn_bar = excel.at[junc, "pn_bar"], tfluid_k = excel.at[junc, "tfluid_k"], height_m = excel.at[junc, "height"], name = excel.at[junc, "name"])
                    self.grid_elem["junctions"].append(excel.at[junc, "name"])

            if self.sheets[i] == "pipe":
                  excel = pd.read_excel(self.Excel_file, sheet_name = self.sheets[i], index_col = 0)
                  for pipe in excel.index:
                      for f in excel.index:
                          from_j = excel.at[pipe, "from_junction"]
                          if from_j in self.grid_elem["junctions"]:
                              from_junc = globals()["junction" + str(f)]  
                          else:
                              pass
                      for t in excel.index:
                          to_j = excel.at[pipe, "to_junction"]
                          if to_j in self.grid_elem["junctions"]:
                              to_junc = globals()["junction" + str(t)]  
                          else:
                              pass    
                      globals()["pipe" + str(pipe)] = ppp.create_pipe(self.net, from_junction = from_junc, to_junction = to_junc, std_type = excel.at[pipe, "std_type"], length_km = excel.at[pipe, "length"], name = excel.at[pipe, "name"])

            if self.sheets[i] == "valve":
                excel = pd.read_excel(self.Excel_file, sheet_name = self.sheets[i], index_col = 0)
                for valve in excel.index:
                    for f in excel.index:
                        from_j = excel.at[valve, "from_junction"]
                        if from_j in self.grid_elem["junctions"]:
                            from_junc = globals()["junction" + str(f)]  
                        else:
                            pass
                    for t in excel.index:
                        to_j = excel.at[valve, "to_junction"]
                        if to_j in self.grid_elem["junctions"]:
                            to_junc = globals()["junction" + str(t)]  
                        else:
                            pass    
                    globals()["valve" + str(valve)] = ppp.create_valve(self.net, from_junction = from_junc, to_junction = to_junc,  diameter_m = excel.at[valve, "diameter"], name = excel.at[valve, "name"])

            if self.sheets[i] == "sink":
                excel = pd.read_excel(self.Excel_file, sheet_name = self.sheets[i], index_col = 0)
                for sink in excel.index:
                    s_j = excel.at[sink, "junction"]
                    if s_j in self.grid_elem["junctions"]:
                        sink_junct = globals()["junction" + str(sink)]
                        globals()["sink" + str(sink)] = ppp.create_sink(self.net, junction = sink_junct, mdot_kg_per_s = excel.at[sink, "mdot_kg_per_s"], name = excel.at[sink, "name"])  

            if self.sheets[i] == "source":
                excel = pd.read_excel(self.Excel_file, sheet_name = self.sheets[i], index_col = 0)
                for source in excel.index:
                    s_j = excel.at[source, "junction"]
                    if s_j in self.grid_elem["junctions"]:
                        source_junct = globals()["junction" + str(source)]
                        globals()["source" + str(source)] = ppp.create_source(self.net, junction = source_junct, mdot_kg_per_s = excel.at[source, "mdot_kg_per_s"], name = excel.at[source, "name"])  
            ...
            if self.sheets[i] == "ext_grid":
                ...
                for ext_grid in excel.index:
                    ...
                    if e_j in self.grid_elem["junctions"]:
                        ext_junct = globals()["junction" + str(ext_grid)]
                        globals()["ext_grid" + str(ext_grid)] = ppp.create_ext_grid(self.net, junction = ext_junct, p_bar = excel.at[ext_grid, "p_bar"], t_k = excel.at[ext_grid, "t_k"], name = excel.at[ext_grid, "name"]) 
        print(self.net)
        return self.net